...
namespace sce
{
	...
	int Engine::negamax(libchess::Position &pos, int depth, int alpha, int beta, Color color, SearchInfo *info) noexcept
	{
		...
		if (ttEntry->hash == pos.hash() && ttEntry->depth >= depth && !info->is_root)
		{
			...
			else if (ttEntry->type == TTEntryType::HF_ALPHA)
			{ // means entry was never bigger than alpha.
				if (scr <= alpha)
					return scr;
				else
					beta = std::min(beta, scr);
			}
			else if (ttEntry->type == TTEntryType::HF_BETA)
			{ // means entry was never smaller than beta.
				if (scr >= beta)
					return scr;
				else
